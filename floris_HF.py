...
def AEP_objective_hf(x,parallelize=True):
    # TODO: parallelize this using multiprocessing
#     from multiprocessing import Pool
...
if __name__ == '__main__':
    ...
    # x_test = [0,0.2,0.4,0.6,0.8,0.95,0,0.2,0.4,0.6,0.8,0.95] 
    # AEP_hf = AEP_objective_hf(x_test)
    # print(f'The normalized AEP for the given layout is {AEP_hf}')

    # AEP_lf = AEP_objective_lf(x_test)

    # print(f'The normalized AEP for the given layout is {AEP_lf}')

    # cons_hf = constraint_hf(x_test)

    # print(f'The constraint for the given layout is {cons_hf}')

    # cons_lf = constraint_lf(x_test)

    # print(f'The constraint for the given layout is {cons_lf}')

    # x_tmp = [0,0.1,0.4,0.6,0.8,0.95,0,0.2,0.4,0.6,0.8,0.95]

    # aep_ = AEP_objective_hf(x_tmp)
    # print(f'The normalized AEP for the different layout is {aep_}')
